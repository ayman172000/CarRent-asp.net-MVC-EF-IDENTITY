...
namespace CarRentt.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Register(RegisterViewModel model, HttpPostedFileBase imgCinRec, HttpPostedFileBase imgCinVer, HttpPostedFileBase imgPerRec, HttpPostedFileBase imgPerVer)
        {
            if (ModelState.IsValid)
            {
                string path = Path.Combine(Server.MapPath("~/Image/cin"), imgCinRec.FileName);
                imgCinRec.SaveAs(path);
                path = Path.Combine(Server.MapPath("~/Image/cin"), imgCinVer.FileName);
                imgCinVer.SaveAs(path);
                path = Path.Combine(Server.MapPath("~/Image/permis"), imgPerRec.FileName);
                imgPerRec.SaveAs(path);
                path = Path.Combine(Server.MapPath("~/Image/permis"), imgPerVer.FileName);
                imgPerVer.SaveAs(path);
                var user = new ApplicationUser {
                    UserName = model.Email,
                    Email = model.Email,
                    ImageCinRecto = imgCinRec.FileName,
                    ImageCinVerso = imgCinVer.FileName,
                    ImagePermisRecto= imgPerRec.FileName,
                    ImagePermisVerso= imgPerVer.FileName,
                    etat="Non Verifié",
